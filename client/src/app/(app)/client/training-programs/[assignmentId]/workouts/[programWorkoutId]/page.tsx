...
import { findAssignedWorkout } from "@/features/training-programs/model/client-assigned-program-selectors";
...
import { ClientAssignedWorkoutDetailView } from "@/features/training-programs/ui/client-assigned-workout-detail-view";
import { isApiError } from "@/lib/api/api-error";

interface ClientAssignedWorkoutDetailsPageProps {
  params: Promise<{
    assignmentId: string;
...
export default async function ClientAssignedWorkoutDetailsPage({
  params,
  searchParams,
}: ClientAssignedWorkoutDetailsPageProps) {
  ...
  const { assignmentId, programWorkoutId } = await params;
  ...
  const fallback = `/client/training-programs/${assignmentId}?week=${workout.weekNumber}`;
  const backHref = getSafeReturnPath(returnTo, fallback);
  const currentHref = buildClientWorkoutPath(assignmentId, programWorkoutId, backHref);

  return (
    <ClientAssignedWorkoutDetailView
