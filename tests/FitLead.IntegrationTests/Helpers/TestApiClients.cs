...
public sealed class TestApiClients(IntegrationTestFixture fixture)
{
    ...
    public async Task<TrainerDashboardTestClient> TrainerDashboardAsync(AuthTestClient auth)
    {
        var client = new TrainerDashboardTestClient(fixture.CreateClient(handleCookies: false));
        await client.CopyAuthStateFromAsync(auth);
        return client;
    }

    public async Task<ClientDashboardTestClient> ClientDashboardAsync(AuthTestClient auth)
    {
        var client = new ClientDashboardTestClient(fixture.CreateClient(handleCookies: false));
        await client.CopyAuthStateFromAsync(auth);
        return client;
    }
