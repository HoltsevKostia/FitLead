...
{
    public sealed class UpdateClientBodyMetricEntryHandler
        : IRequestHandler<UpdateClientBodyMetricEntryCommand, Result<ClientBodyMetricEntryDto>>
    {
        ...
        public UpdateClientBodyMetricEntryHandler(
        ...
        public async Task<Result<ClientBodyMetricEntryDto>> Handle(
            UpdateClientBodyMetricEntryCommand request,
            CancellationToken cancellationToken)
        {
            ...
            var duplicateExists = await _repository.ExistsForClientRecordedAtAsync(
                clientId,
                request.RecordedAt,
                excludeEntryId: request.EntryId,
                cancellationToken);
            if (duplicateExists)
            {
                return Result<ClientBodyMetricEntryDto>.Failure(
            ...
            {
                return Result<ClientBodyMetricEntryDto>.Failure(updateResult.Error);
            }

            await _unitOfWork.SaveChangesAsync(cancellationToken);

            return Result<ClientBodyMetricEntryDto>.Success(
                ClientBodyMetricEntryMapping.ToDto(entry));
        }
    }
}
