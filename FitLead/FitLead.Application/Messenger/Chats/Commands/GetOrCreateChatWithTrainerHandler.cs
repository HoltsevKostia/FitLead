...
{
    public sealed class GetOrCreateChatWithTrainerHandler
        : IRequestHandler<GetOrCreateChatWithTrainerCommand, Result<ChatDto>>
    {
        ...
        public GetOrCreateChatWithTrainerHandler(
        ...
        public async Task<Result<ChatDto>> Handle(
            GetOrCreateChatWithTrainerCommand request,
            CancellationToken cancellationToken)
        {
            var accessResult = await _chatLoader.EnsureCurrentClientHasTrainerAsync(
                request.TrainerId,
                cancellationToken);
